...
function parallaxSlider(options) {
    var $ = jQuery
    this.setting = {displaymode: {type: 'auto', pause: 1000, stoponclick: false, cycles: 2, pauseonmouseover: false}, delaybtwdesc: 500, activeslideclass: 'selectedslide', orientation: 'h', persist: true, slideduration: 500} //default settings
...
//                $(this).append('<div class="bullets">bullet</div>')
    })
...
//        console.log($(this));
        ...
        $(this.$bullets.find('i')).removeClass('active');
        $($(this.$bullets.find('i'))[nextslide]).addClass('active');
//        $($('.bullets>i')[nextslide]).addClass('active');
    ...
    init: function ($) {
        ...
        this.$imageslides.each(function (i) {
            if (index == 0) {
                $bullets.append('<i class="active"></i>')
            } else {
                $bullets.append('<i></i>')
            }
            index++;
        ...
        this.$bullets = $bullets.appendTo(this.$wrapperdiv)

        var t = this;

        this.$bullets.find('i').click(function () {
            if (t.animation_isrunning)
                return;
            if ($(this).index() > t.curslide) {
                t.slide($(this).index(), "left");
            }
            if ($(this).index() < t.curslide) {
                t.slide($(this).index(), "right");
            }
